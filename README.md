...
This project abandons object-oriented loops in favor of a strict **Neural Cellular Automaton (NCA)**. The entire ecosystem is a single contiguous tensor where the laws of physics and biology are reduced to simultaneous 2D convolutions.
...
1. **Wight Inspector:** Hover your mouse over the ecosystem to lock onto a single living wight. The inspector permanently tracks that organism's geographic coordinates over time, decodes its 15-channel brain into a real-time bar graph, and retains post-mortem diagnostics (`[DEAD]`) when its energy inevitably hits zero.
...
4. **Emergence Log:** A scrolling real-time event ticker dynamically tags macro-evolutionary milestones as they occur—like population bottlenecks, metabolism breakthroughs, or biological immortality.

When the matrix boots, the population initializes as a single noisy cloud. As natural selection filters the world tensor, you can watch the math forcefully tear itself apart into distinct, autonomous islands. 

These clusters represent ecological niches forming natively within the latent space—such as an island of high-mobility scavengers branching away from a dense cluster of stationary lurkers. You are viewing speciation strictly as a byproduct of mathematical optimization.
